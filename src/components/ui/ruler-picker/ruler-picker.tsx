...
const RulerPicker: React.FC<RulerPickerProps> = ({
  ...
  const CENTER_OFFSET = useMemo(
    () => (viewWidth - ITEM_WIDTH) / 2,
    [viewWidth]
  );

  const scaleData = useMemo(() => {
    const length = Math.floor((maxValue - minValue) / step) + 1;
    return Array.from({ length }, (_, i) =>
      Number((minValue + i * step).toFixed(1))
    );
  ...
  const getOffsetX = (index: number) => {
    ...
    onValueChange?.(value);
    return (
      index * ITEM_WIDTH - CENTER_OFFSET + (viewWidth / 2 - ITEM_WIDTH / 2)
    );
  };
  ...
  const handleScrollEnd = useCallback(
    (event: any) => {
      ...
      const newOffsetX = getOffsetX(index);
      scrollViewRef.current?.scrollTo({ x: newOffsetX, animated: true });
    },
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [CENTER_OFFSET, scaleData, viewWidth, onValueChange]
  );

  useLayoutEffect(() => {
    const initialIndex = Math.round((initialValue - minValue) / step);
    const newOffsetX = getOffsetX(initialIndex);
    setTimeout(() => {
      scrollViewRef.current?.scrollTo({ x: newOffsetX, animated: false });
    }, 0);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [CENTER_OFFSET, minValue, step, viewWidth, initialValue]);
  ...
  return (
    <View onLayout={handleLayout} style={[styles.container, rulerStyle]}>
