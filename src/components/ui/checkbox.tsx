...
import { colors, type Font } from '@/lib';
...
const Label = ({ text, testID, font, className = '' }: LabelProps) => {
  return (
    <Text testID={testID} font={font} className={` ${className} pl-2`}>
      {text}
    </Text>
  );
};
...
const CheckboxBase = ({
  ...
  labelFont,
  ...props
}: RootProps & { label?: string; labelFont?: Font }) => {
  return (
